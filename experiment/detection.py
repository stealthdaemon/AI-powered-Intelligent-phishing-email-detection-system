            ...
            for part, part_info in zip(base64_parts, base64_parts_info):
                ...
                if part_info['validate'] == "valid":  # check if the decoded data is valid
                    ...
                    if part_info['Content-Type'].startswith("image/"):
                        ...
                        info = "text/plain extracted from decoded base64 image (ignore if scrambled or correct)"
...
def store_result(dict_file, filename, path):
    ...
    data = {}
    try:
        with open(f'phishinganalysis.json', 'r') as file:
            json_file = file.read().strip()
            if json_file:
                data = json.loads(json_file)
    ...
    if dict_file[filename]['is_phishing'] is True:
        shutil.move(path, os.path.join(f'processed_emails/phishing', filename))
    elif dict_file[filename]['is_phishing'] is False:
        shutil.move(path, os.path.join(f'processed_emails/ham', filename))
    else:
        shutil.move(path, os.path.join(f'processed_emails/uncertain', filename))

    # Write the updated data back to file
    with open(f'phishinganalysis.json', 'w') as file:
        json.dump(data, file, indent=4)
...
def main(path):
    try:
        ...
        if phishing_result[filename]['is_phishing'] is True:
            phishing_result[filename]['user_feedback'] = 'correct'
        else:
            phishing_result[filename]['user_feedback'] = 'wrong'
